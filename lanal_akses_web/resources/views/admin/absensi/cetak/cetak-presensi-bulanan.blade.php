    ...
    @endphp
     ...
     <table>
        <tr>
            ...
            <td>{{ \Carbon\Carbon::now()->translatedFormat('F') }}</td>
        </tr>
        <tr>
            <th>Jenis Presensi</th>
            <td>Presensi Bulanan</td>
        </tr>
    </table>
    <div>
...
<table>
    <thead>
        <tr>
            ...
            <th rowspan="2">NRP</th>
            <th colspan="{{ $jumlahHari }}">Rekap Kehadiran Bulan {{ \Carbon\Carbon::now()->translatedFormat('F') }}</th>
    ...
    <tbody>
        @foreach ($absensiPersonil->groupBy('personil_id') as $personilId => $kehadiranPersonil)
            <tr>
                ...
                @for($i = 1; $i <= $jumlahHari; $i++)
                    @php
                        $tanggal = $startDate->copy()->addDays($i-1)->format('Y-m-d');
