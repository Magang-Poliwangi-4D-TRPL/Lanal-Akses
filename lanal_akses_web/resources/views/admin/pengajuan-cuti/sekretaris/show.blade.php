...
<div class="container ">
    <div class="container-fluid bg-white border rounded p-4 mt-4 ">
        <div class="container-fluid p-0 border-bottom">
            ...
            <div class="row py-3 align-items-center">
                <div class="col-md-6 align-items-start">
                    ...
                    @if ($suratPengajuan->responCuti->status_komandan == 'Disetujui')
                        <a href="{{ route('admin.surat-cuti.sekretaris.editNoSurat', $suratPengajuan->id) }}" class="btn btn-primary">Edit Nomor Surat</a>
                    @else
        ...
        @if ($suratPengajuan->responCuti->status_sekretaris !== 'Menunggu Persetujuan')
            <p class="mt-3">Surat Pengajuan ini telah <span class="p-2 rounded {{ $statusClass }} text-center">{{ $suratPengajuan->responCuti->status_sekretaris }}</span> oleh anda.</p>
        @else
             
        <div class="container py-4">
            <form method="POST" action="{{ route('admin.surat-cuti.sekretaris.createRespon', $suratPengajuan->id) }}">
                @csrf
                <div class="form-group">
                    <label for="status_sekretaris">Respon Anda</label>
                    <select class="form-control @error('status_sekretaris') is-invalid @enderror" id="status_sekretaris" name="status_sekretaris" required>
                        <option value="">Pilih Respon Anda</option>
                        <option value="Disetujui" {{ old('status_sekretaris') == 'Disetujui' ? 'selected' : '' }}>Disetujui</option>
                        <option value="Ditolak" {{ old('status_sekretaris') == 'Ditolak' ? 'selected' : '' }}>Ditolak</option>
                    </select>
                    @error('status_sekretaris')
                ...
                <div class="form-group">
                    <label for="keterangan_sekretaris">Keterangan</label>
                    <input type="text" class="form-control @error('keterangan_sekretaris') is-invalid @enderror" value="{{ old('keterangan_sekretaris') }}" id="keterangan_sekretaris" name="keterangan_sekretaris" placeholder="Masukkan keterangan anda (tidak wajib)">
                    @error('keterangan_sekretaris')
                    <div class="alert alert-danger" role="alert">
                        <p class="p-0 m-0">{{ $message }}</p>
                    </div>
                    @enderror
                ...
                <div class="form-group">
                    ...
                    <input type="text" class="form-control @error('nomor_surat') is-invalid @enderror" value="{{ old('nomor_surat') }}" id="nomor_surat" name="nomor_surat" placeholder="Masukkan nomor surat (tidak wajib)">
                    @error('nomor_surat')
                    <div class="alert alert-danger" role="alert">
                        <p class="p-0 m-0">{{ $message }}</p>
                    </div>
