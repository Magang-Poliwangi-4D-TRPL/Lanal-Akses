...
<div class="container ">
    <div class="container-fluid bg-white border rounded p-4 mt-4 ">
        ...
        @if ($suratPengajuan->responCuti->status_atasan !== 'Menunggu Persetujuan')
            <p class="mt-3">Surat Pengajuan ini telah <span class="p-2 rounded {{ $statusClass }} text-center">{{ $suratPengajuan->responCuti->status_atasan }}</span> oleh anda.</p>
        @else
             
        <div class="container py-4">
            <form method="POST" action="{{ route('admin.surat-cuti.satker.createRespon', $suratPengajuan->id) }}">
                @csrf
                <div class="form-group">
                    <label for="status_atasan">Respon Anda</label>
                    <select class="form-control @error('status_atasan') is-invalid @enderror" id="status_atasan" name="status_atasan" required>
                        <option value="">Pilih Respon Anda</option>
                        <option value="Disetujui" {{ old('status_atasan') == 'Disetujui' ? 'selected' : '' }}>Disetujui</option>
                        <option value="Ditolak" {{ old('status_atasan') == 'Ditolak' ? 'selected' : '' }}>Ditolak</option>
                    </select>
                    @error('status_atasan')
