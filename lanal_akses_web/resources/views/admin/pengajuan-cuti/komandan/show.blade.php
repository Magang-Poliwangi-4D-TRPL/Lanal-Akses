...
<div class="container ">
    <div class="container-fluid bg-white border rounded p-4 mt-4 ">
        ...
        @if ($suratPengajuan->responCuti->status_komandan !== 'Menunggu Persetujuan')
            <p class="mt-3">Surat Pengajuan ini telah <span class="p-2 rounded {{ $statusClass }} text-center">{{ $suratPengajuan->responCuti->status_komandan }}</span> oleh anda.</p>
        @else
             
        <div class="container py-4">
            <form method="POST" action="{{ route('admin.surat-cuti.komandan.createRespon', $suratPengajuan->id) }}">
                @csrf
                <div class="form-group">
                    <label for="status_komandan">Respon Anda</label>
                    <select class="form-control @error('status_komandan') is-invalid @enderror" id="status_komandan" name="status_komandan" required>
                        <option value="">Pilih Respon Anda</option>
                        <option value="Disetujui" {{ old('status_komandan') == 'Disetujui' ? 'selected' : '' }}>Disetujui</option>
                        <option value="Ditolak" {{ old('status_komandan') == 'Ditolak' ? 'selected' : '' }}>Ditolak</option>
                    </select>
                    @error('status_komandan')
                ...
                <div class="form-group">
                    <label for="keterangan_komandan">Keterangan</label>
                    <input type="text" class="form-control @error('keterangan_komandan') is-invalid @enderror" value="{{ old('keterangan_komandan') }}" id="keterangan_komandan" name="keterangan_komandan" placeholder="Masukkan keterangan anda (tidak wajib)">
                    @error('keterangan_komandan')
                    <div class="alert alert-danger" role="alert">
                        <p class="p-0 m-0">{{ $message }}</p>
                    </div>
                    @enderror
