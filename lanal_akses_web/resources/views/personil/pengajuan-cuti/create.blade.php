        ...
        <div class="container py-4">
            <form method="POST" action="{{ route('personil.pengajuan-cuti.store') }}">
                ...
                <div class="form-group">
                    <label for="personil_id">Nama Personel</label>
                    <select class="form-control @error('personil_id') is-invalid @enderror" id="personil_id" name="personil_id" required readonly>
                        <option value="{{ auth()->user()->personil->id }}">{{ auth()->user()->personil->nama_lengkap }}</option>
                    </select>
                    @error('personil_id')
                ...
                <div class="form-group">
                    ...
                    @foreach ($dataCuti as $cuti)
                        <option value="{{ $cuti->kode_cuti }}" {{ old('jenis_cuti') == $cuti->kode_cuti ? 'selected' : '' }}>{{ ' [' . $cuti->kode_cuti.'] ' .$cuti->nama_cuti . ' sisa cuti anda : ' . $cuti->sisa_hari_cuti }}</option>
                    @endforeach
                    </select>
                    @error('jenis_cuti')
                    <div class="alert alert-danger" role="alert">
